 ...
 * 2017/11/13 rwp
 *    Changed gpsd support to be conditionally compiled
 * 2018/08/31 rwp
 *    Changed config from local_features to ConfigManager
 * 2019/03/11 rwp
 *    Added GPS and GNSS sentence support
 * 2019/03/13 rwp
 *    Combined gps_mon and gps_mon-eth
 * 2019/08/21 rwp
 *    Added listen socket to support sending NMEA sentences
 *    Condition out SpeedLimits, GeoFences and FleetTracker
 ******************************************************************************
 */
...
void sendToReaders(string line);
void update();
...
void openEthPort(string ip, int port);
int listenSocket(string name);
void onTerm(int parm);
void onHup(int parm);

string serial_device = "usbgps";
string serial_speed = "4800";
string gpsd_port = "2947";
bool debug = false;
...
int main(int argc, char *argv[])
{
    ...
    bool debug_option = opts.exists("d");
    if (debug_option) {
        debug = true;
    }
    use2d = opts.exists("2");
    ...
    if (config.exists("gps.SerialDevice")) {
        serial_device = config.getString("gps.SerialDevice");
    }
    if (config.exists("gps.SerialSpeed")) {
        serial_speed = config.getString("gps.SerialSpeed");
    }
    if (config.exists("gps.GpsdPort")) {
        gpsd_port = config.getString("gps.GpsdPort");
        use_gpsd = true;
    }
    ...
    if (config.exists("gps.ListenPort")) {
        listenPort = config.getString("gps.ListenPort");
    }
    ...
    testing_acc = features.exists("DebugAcc");
    if (argc > 1) {
        serial_device = argv[1];
    }
    ...
    while (true) {
        if (demo_mode) {
            logError("Entering DemoMode");
            demoGps();
        }
        else if (use_eth) {
            if (streamgps) {
                logError("Entering EthGpsStreamingMode");
                ethGpsStreaming();
            }
            else {
                logError("Entering EthGpsMode");
                ethGps();
            }
        }
#ifdef GPSD_SUPPORT
        else if (use_gpsd) {
            logError("Entering GpsdMode");
            gpsdGps();
        }
#endif
        else if (fileExists("/dev/"+serial_device)) {
            logError("Entering SerialMode: "+serial_device+"/"+serial_speed);
            monitorGps();
        }
        sleep(1);
    }
...
void monitorGps() {
    ...
    if (listenPort != "") {
        listen_s = listenSocket(listenPort);
        if (listen_s < 0) {
            logError("Could not open listenPort: "+listenPort);
        }
    }
...
#ifdef FLEET_TRACKER
            if ((fltsock != -1) and FD_ISSET(fltsock, &rd)) {
                struct sockaddr_in from;
                socklen_t fromlen = sizeof(from);
                char buffer[128];
                int n = recvfrom(fltsock, buffer, sizeof(buffer-1), 0, (struct sockaddr *)&from, &fromlen);
                if (n > 0) {
                    buffer[n] = '\0';
...
void sendToReaders(string line) {
    ...
    for (ri=readers.begin(); ri != readers.end(); ri++) {
        ...
        bool opened = (*ri).second;
        if (opened) {
            int len = send(rs, line.c_str(), line.length(), 0);
            if (len == 0) {
                close(rs);
                readers[rs] = false;
            }
...
int gpsdGps(void) {
    int rc;
    bool have_tried_to_open_gpsd = false;

    while(true) {
        if (!have_opened_gpsd) {
            if (!have_tried_to_open_gpsd)
               logError("opening gpsd socket");
            if ((rc = gps_open("localhost", gpsd_port.c_str(), &_gps_data)) == -1) {
                if (!have_tried_to_open_gpsd)
                   fprintf(stderr, "code: %d, reason: %s\n", rc, gps_errstr(rc));
                //return EXIT_FAILURE;
                have_tried_to_open_gpsd = true;
                sleep(1);
                continue;
            }
            logError("opened gpsd socket");
            gps_stream(&_gps_data, WATCH_ENABLE | WATCH_JSON, NULL);
            //gps_stream(&_gps_data, WATCH_ENABLE | WATCH_NMEA, NULL);

            have_tried_to_open_gpsd = false;
            have_opened_gpsd = true;
         }
         if (_gpsdGps() < 0) {
            logError("closing gpsd socket");
            // close socket
...
int _gpsdGps() {
    int rc;
    static int iGpsdStuckCnt = 0;
 
    /* wait for 5 seconds to receive data */
    if (gps_waiting (&_gps_data, 5000000)) {
        /* read data */
...
/*
            // put in to hanlde nmea sentences
            {
               const char* buffer = NULL;
               char sentence[1024];
               int i;

               buffer = gps_data(&_gps_data);
if (!buffer) cerr << "buffer == zero" << endl;
               if (!buffer)
                   return 0;
               i = strlen(buffer);
if (i == 0) cerr << "i == zero" << endl;
               if (i >= sizeof(sentence))
                  i = sizeof(sentence)-1;
               strncpy(sentence, buffer, i);
               sentence[i]= 0;
               if (i < 2)
                  sentence[0] = 0;
               else
                  sentence[i-1]=0;  // delete cr
  
               if ((*sentence != 0) && !strchr(sentence, '\r') && strchr(sentence, '*')) {
...
*/
            ...
            if ((_gps_data.status == STATUS_FIX) &&
                ...
                !isnan(_gps_data.fix.longitude)) {
                    gpsinfo.latitude = _gps_data.fix.latitude;
                    gpsinfo.longitude = _gps_data.fix.longitude;
                    gpsinfo.speed = _gps_data.fix.speed * 1.943844; // m/s to knots
                    gpsinfo.track = _gps_data.fix.track;
                    if (gpsinfo.track < 0.0) { // relative to north to 0-359.9
                       gpsinfo.track += 360.0;
                    }
                    //gpsinfo.fix3d = (_gps_data.fix.mode == MODE_2D) ? 2 : 3;
                    gpsinfo.fix3d = (_gps_data.fix.mode == MODE_2D) ? 3 : 3;
            } else {
                gpsinfo.fix3d = 0;
            }
            update();
    ...
    iGpsdStuckCnt++;
    if (!(iGpsdStuckCnt % 12)) {
        // haven't rcv'd any data from gpsd for a minute.
        // close and reopen the port
        fprintf(stderr, "error no data for one minute!!\n");
        return -1;
    } else {
        // gps_stream disables itself after a few seconds..
...
void ethGps(void) {
    while(true) {
        if (!have_opened_eth) {
            openEthPort(gpstalker_ip, gpstalker_port);
...
//    if (!got_position) cerr << line << endl;
    ...
    if ( (gpsinfo.last_hr_time != 0.0) and
        ...
        gpsinfo.gngga_seen = false;
        gpsinfo.gnrmc_seen = false;
        gpsinfo.gngsa_seen = false;
...
//            }
    ...
    if ( (gpsinfo.gpgga_seen or gpsinfo.gngga_seen) and
         (gpsinfo.gpgsa_seen or gpsinfo.gngsa_seen) and
         (gpsinfo.gprmc_seen or gpsinfo.gnrmc_seen) ) {
        checkTime();
        if (testing_acc) {
/*
            Byte buffer[64];
...
void openPort(const char *port) {
    ...
    int baud = baudString(serial_speed);
    newtio.c_cflag = baud | CS8 | CSTOPB | CLOCAL | CREAD;
...
int baudString(string baud) {
    ...
    if (baud == "2400") {
        ...
        logError("Invalid serial port speed: "+baud);
    }
}
...
string getResponse(string command, string ans, int timeout) {
    ...
    alarm(60);
    if (write(eth_s, cmndstr.c_str(), cmndstr.size()) <= 0) {
        closeAndExit(1);
    }
    char last_ch = '\n';
    string line = "";
    while (!done) {
        ...
        if (r > 0) {
            if (FD_ISSET(eth_s, &rd)) {
                char buf[255];
                int res = read(eth_s,buf,255-1);
                int i;
                if (res>0) {
                    for (i=0; i < res; i++) {
                        char ch = buf[i];
...
void openEthPort(string ip, int port) {
    ...
    eth_s=socket(AF_INET,SOCK_STREAM,0);
    ...
    while (connect(eth_s,(struct sockaddr *)&servaddr,sizeof(servaddr)) < 0) {
...
int listenSocket(string name) {
    ...
    if (name[0] == '/') {
        ...
        chmod(name.c_str(), S_IRUSR | S_IWUSR | S_IROTH | S_IWOTH | S_IRGRP | S_IWGRP);
    }
    else {
        struct sockaddr_in a;
        int port = stringToInt(name);
