...
//    readHardwareInfo();
    ...
    if (lte_model) {
        string profile_resp = getResponse("getprofile",1);
        if (profile_resp.find("iccid:") != string::npos) {
            //iccid = getField(profile_resp,"iccid:",1);
            iccid = getField(profile_resp,",",1);
            iccid = iccid.substr(6);
            if (profile_resp.find("sim:ready") != string::npos) {
                simok = true;
            }
        }
    }
...
//                        readers.erase(rs);
                        ...
                        string resp = getResponse(read_buf, 5);
                        resp += "\n";
...
void updateHardwareInfo() {
    ...
    hdw_info["cell_device"] = "eth0";
    hdw_info["cell_islte"] = "true";
    hdw_info["cell_talker"] = "sw_talker-eth";
    hdw_info["cell_ping_interval"] = "30";
...
string lte_getprofile() {
    ...
    if (iccid == "") {
        ...
        outstr = getResponse("getprofile", 5);
    }
    return outstr;
}
...
string getmodel() {
    string outstr = "";
    string model = getResponse("getmodel", 5);
...
string data_connect() {
    string resp = getResponse("connect", 2);
...
string data_disconnect() {
    string outstr = "Failed";
    string resp = getResponse("AT!DATALINUX=0,1", 2);
    if (resp == "OK") {
        outstr = "OK";
    }
    return outstr;
...
string isconnected() {
    string outstr = "no";
    string resp = getResponse("AT!DATALINUX?1", 4);
    if (resp.find("ONLINE") != string::npos) {
        outstr = "yes";
    }
    //else {
...
//                            }
                    ...
                    cerr << " socket closed or error" << endl;
                    closeAndExit(1);
                }
            }
            if (busy_resp != "") {
                ...
                for (ri=readers.begin(); ri != readers.end(); ri++) {
                    ...
                    if (set and FD_ISSET(rs, &rd)) {
                        alarm(1);
                        char read_buf[255];
                        int res = read(rs, read_buf, 255-1);
                        alarm(0);
                        if (res > 0) {
                            read_buf[res] = '\0';
                            while ( (read_buf[res-1] == '\n') or
