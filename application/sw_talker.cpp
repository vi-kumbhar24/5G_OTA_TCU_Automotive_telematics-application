 ...
 * 2019/04/25
 *   Added ability to get MDN from cell_shell getprofile
 ******************************************************************************
 */
...
//            resetIt();
    ...
    else if (beginsWith(model,"AR7592")) {
        lte_model = true;
        has_gps = true;
        enable_extended_cmds();
    }
    else if (beginsWith(model,"AR7594")) {
        lte_model = true;
        has_gps = true;
        enable_extended_cmds();
    }
...
void startGps() {
    string gpsautostart = getResponse("AT!GPSAUTOSTART?");
    string function = getParm(gpsautostart, "function:  ");
    if (function == "0") {
        (void)getResponse("AT!GPSAUTOSTART=1,1,180,50,1");
        (void)getResponse("AT!GPSTRACK=1,180,50,1000,1");
    }
}

string stopgps() {
    string resp = "";
    resp = getResponse("AT1GPSEND=0",3);
    if (resp != "OK") {
        resp = "ERROR";
    }
    return resp;
