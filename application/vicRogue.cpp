 ...
 * 2014/12/02 rwp
 *   Added gettime command
 *   Added execution of multiple commands
 * 2015/03/31 rwp
 *   Added getall command
 * 2015/06/30 rwp
 *   Added setwakeup command
 ***************************************************************************
 */
...
void setTime();
void getVersion();
void setWakeup(string yymmdd, string hhmmss);
void getAll(Byte last);
void getStatus();
void getLastStatus();
...
void ecuLearning();
void doRogue(string lights, string wiper, string horn, string speed, string rpm);
...
//    vic.openSocket(1,notifications);
    ...
    if (argc < 2) {
        ...
        cout << "    rogue <lights> <wiper> <horn> <speed> <rpm>" << endl;
        exit(0);
    }
    if (!nosettime) {
        getTime();
    ...
    for (int idx=1; idx < argc; idx++) {
        ...
        if (arg1 == "cellreset") {
            ...
            if (arg6 == "") {
                ...
                doRogue(arg2, arg3, arg4, arg5, arg6);
            }
            idx += 5;
        }
...
void setWakeup(string yymmdd, string hhmmss) {
    Byte buffer[256];
    cout << "Setting wakeup time: " << yymmdd << " " << hhmmss << endl;
    buffer[0] = stringToInt(yymmdd.substr(0,2));
    buffer[1] = stringToInt(yymmdd.substr(2,2));
    buffer[2] = stringToInt(yymmdd.substr(4,2));
    buffer[3] = stringToInt(hhmmss.substr(0,2));
    buffer[4] = stringToInt(hhmmss.substr(2,2));
    buffer[5] = stringToInt(hhmmss.substr(4,2));
    vic.sendRequest(app_id, 0x0E, 6, buffer);
    int len = getExpectedResponse(0x0E, buffer, 2, true);
}
...
void getAll(Byte last) {
    ...
    int len = getExpectedResponse(0x35, buffer, 32, false);
    if (len > 0) {
        int idx = 2;
        ...
        idx += 1;
//        decodeSpeed(buffer+idx);
//        idx += 2;
//        decodeOdom(buffer+idx);
//        idx += 3;
...
void decodeStatus(Byte *buffer) {
    ...
    Byte locks = buffer[1];
    Byte doors = buffer[2];
    unsigned int ign_time = ((int)buffer[3] << 8) | buffer[4];
    Byte batvalx10 = buffer[5];
    Byte vic_status_code = buffer[6];
    string vic_status = "";
    ...
    if (vic.protocol_version >= 0x0214) {
//    if (len > 9) {
        int more_state = (buffer[7] << 8) | buffer[8];
        int car_capabilities = (buffer[9] << 8) | buffer[10];
...
void decodeTires(Byte buffer[]) {
    if ( (buffer[2] != 0xFF) and
         ...
         (buffer[4] != 0xFF) and
         (buffer[5] != 0xFF) ) {
        int left_front = buffer[2];
        int right_front = buffer[3];
        int left_rear = buffer[4];
        int right_rear = buffer[5];
        cout << "Tire pressures:" << endl;
        if ((buffer[0] != 0xFF) and (buffer[1] != 0xFF)) {
            int front_ideal = buffer[0];
            int rear_ideal = buffer[1];
...
void decodeFuelUsage(Byte buffer[]) {
    ...
    int units = buffer[6];
    unsigned int raw_current = (buffer[7] << 8) + buffer[8];
    cout << "Raw current: " << raw_current << endl;
    double ul_total = 0.;
    double ul_current = 0.;
    double lps = 0.;
    if (units == 1) {
...
void getSpeed() {
    ...
    if (len > 0) {
        decodeSpeed(buffer+2);
    }
}

void decodeSpeed(Byte buffer[]) {
    speed_vic = ((int)buffer[0]<<8) | buffer[1];
    if (speed_vic != 0xFFFF) {
...
void decodeOdom(Byte buffer[]) {
    long int odom = ((long int)buffer[0]<<16) +
                    ((long int)buffer[1]<<8) +
                    buffer[2];
    if (odom != 0xFFFFFF) {
        double km = odom / 10.;
...
void getSeats() {
    ...
    if (len > 0) {
        decodeSeats(buffer+2);
    }
}

void decodeSeats(Byte buffer[]) {
    int seats = buffer[0];
    int seatbelts = buffer[1];
    if (seats & 0x01) {
...
void doRogue(string lights, string wiper, string horn, string speed, string rpm) {
    Byte buffer[64];
    buffer[0] = 0x00; // lights 0=off 1=on
    buffer[1] = 0x00; // wiper 0=off 1=on
    buffer[2] = 0x00; // wiper 0=off 1=on
    buffer[3] = 0xFF; // wiper 0xFF=off 0-0xA0=speed
    buffer[4] = 0xFF; // wiper 0xFF=off 0x0000-0x1B58=rpm
    buffer[5] = 0xFF;
    if (lights == "on") {
        buffer[0] = 0x01;
    }
    ...
    vic.sendRequest(app_id, 0x80, 6, buffer);
    int len = getExpectedResponse(0x80, buffer, 2, true);
}
